...
router.post('/checkout_action', async (req, res, next) => {
    ...
    const orderDoc = {
        ...
        orderPaymentMessage: 'El pago se realiza manual',
    ...
    try{
        ...
        .then(() => {
            ...
            req.session.message = 'Su pedido fue realizado con éxito. El pago de su pedido se completará manualmente.';
            ...
            common.sendEmail(req.session.paymentEmailAddr, `Su orden con ${config.cartTitle}`, common.getEmailTemplate(paymentResults));

            // redirect to outcome
            res.redirect('/payment/' + newId);
        });
    }catch(ex){
        console.log('Error al enviar el pago a la API', ex);
