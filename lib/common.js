...
const getImageStore = async (id, req, res, callback) => {
    const db = req.app.db;

    const store = await db.stores.findOne({ _id: getId(id) });
    if(!store){
        return [];
    }

    // loop files in /public/uploads/
    const files = await glob.sync(`public/uploads/stores/${store._id.toString()}/**`, { nosort: true });
    ...
    for(let i = 0; i < files.length; i++){
        ...
        if(fs.lstatSync(files[i]).isDirectory() === false){
            ...
            if(store.storeImage === files[i].substring(6)){
                file.storeImage = true;
            }
            // push the file object into the array
            fileList.push(file);
        }
...
const getEmailTemplate = (result) => {
    ...
    $('#paymentMessage').text('Gracias por comprar con nosotros. Esperamos que vuelva a comprar con nosotros pronto.');
...
const paginateDataProduct = (frontend, req, page, collection, query, sort) => {
    const db = req.app.db;
    const config = getConfig();
    let numberItems = 10;
    if(frontend){
