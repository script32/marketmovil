...
const indexStores = (app) => {
    // index all store in lunr on startup
    return new Promise((resolve, reject) => {
        app.db.stores.find({}).toArray((err, storesList) => {
            if(err){
                console.error(colors.red(err.stack));
                reject(err);
            }
            ...
            // setup lunr indexing
            const storesIndex = lunr(function(){
                this.field('email', { boost: 10 });
                this.field('name', { boost: 5 });
                this.field('phone');
                ...
                storesList.forEach((stores) => {
                    const doc = {
                        email: stores.email,
                        name: `${stores.firstName} ${stores.lastName}`,
                        phone: stores.phone,
                        id: stores._id
                    };
                    lunrIndex.add(doc);
            ...
            app.storesIndex = storesIndex;
            if(process.env.NODE_ENV !== 'test'){
                console.log(colors.cyan('- Stores indexing complete'));
            }
            resolve();
        });
    });
